...
\input{figures/arch_tikz_styles.tex}

\newcommand{\repoURL}{https://github.com/MAKhan22/TraceLens-Automated-Debugging}
\newcommand{\repoLink}{\href{\repoURL}{github.com/MAKhan22/TraceLens-Automated-Debugging}}
...
\title{TraceLens: Multimodal Relative Debugging of Failed UI Test Executions via Pass--Fail Trace Comparison}

\author{Musab Ahmed Khan}
\authornote{CS~460 Project, Sabanci University. Student ID: 33017.\\[0.25em]Artifact: \repoLink}
\affiliation{%
...
\begin{abstract}
When an automated UI test fails, engineers often practice \emph{relative debugging}: they compare a known-good execution against the failing run of the \emph{same} test to find the earliest step whose behavior diverged---the likely root cause, not a later symptom.
...
Our first architecture (v1) stacked heuristic ranking, LLM reranking, VLM ensemble, and trace-specific Hit@1 guards; hybrid mode proved brittle when guards tuned for one failure pattern hurt accuracy on others.
We therefore introduce v2, which replaces post-hoc overrides with \emph{unified weighted fusion} and read-only diagnosis.
On a benchmark of 22 real pass--fail UI test trace pairs, a heuristic baseline achieves 64\% Hit@1.
LLM-only reranking reaches \textbf{86\% Hit@1}, the best top-1 localization.
Hybrid LLM+VLM fusion reaches \textbf{82\% Hit@1} but \textbf{100\% Hit@3} with lower mean rank distance (0.27 vs.\ 0.32), recovering visually dominant cases while occasionally losing top-1 when visual noise overrides causal text.
...
\keywords{relative debugging, test failure diagnosis, UI testing, execution traces, fault localization, LLM, VLM}
...
When the same test produces a passing run and a failing run, debugging is inherently \emph{relative}: engineers diff the reference execution against the failure to locate where step behavior first diverged.
Failures include incorrect inputs, wrong navigation, HTTP errors, verification mismatches, and timeouts---not only application crashes or post-release code regressions.
We call this task \textbf{relative debugging}, following the compare-against-reference paradigm of Abramson et al.~\cite{abramson1996relative,abramson1995sc,sosic1997guard}: given a passing trace~$P$ and a failing trace~$F$ for the same test case, rank execution steps by how likely each is the \emph{earliest root cause} of the failed execution.
...
A wrong credential at step~7 may surface only as a verification failure at step~8; ranking by error magnitude favors the symptom~\cite{jones2002visualization}.
...
We evaluate five configurations against a \textbf{heuristic baseline} (pass--fail signal diffing without API calls): a no-pixel ablation, LLM reranking, VLM screenshot analysis, and hybrid fusion combining LLM+VLM.
...
On \texttt{amazon}, the ground-truth fault is an auth-gated click (step~20); text and visual channels can disagree on which step to rank first---we analyze such cases in Section~\ref{sec:eval}.

Section~\ref{sec:eval} reports results for all five configurations on 22 traces and discusses which mode fits different debugging workflows; we do not assume a winner before measurement.

\paragraph{Contributions.}
\begin{itemize}
  \item \textbf{TraceLens}, an end-to-end relative debugging pipeline for pass--fail UI test traces with interpretable multimodal signals and stakeholder reports.
  ...
  \item \textbf{Empirical study} comparing all five configurations on 22 diverse traces, with ablations and per-mode analysis.
\end{itemize}
...
Their SC'95 application to large numerical models~\cite{abramson1995sc} validated the approach on evolving scientific codes.
The \textsc{Guard} tool~\cite{sosic1997guard} operationalized relative debugging for software evolution.
...
Trace comparison also appears in log analysis~\cite{xu2009mining,jones2002visualization} and web testing~\cite{ricca2001testing,mesbah2012crawling}, but prior tools rarely publish reproducible ranking metrics over multimodal UI traces with ground-truth fault steps.
...
Recent surveys document LLM use for bug repair, log parsing, and code generation~\cite{hou2024llm,brown2020language}.
We use an LLM as a \emph{reranker} over compact relative features, not as a patch generator.
Vision-language models enable GUI understanding~\cite{yao2023webarena,deng2023mind2web,zheng2024seeact}; we apply VLMs to \emph{pass vs.\ fail screenshot pairs} at candidate steps identified by cheaper heuristics, controlling API cost.
...
\begin{figure}[t]
  \centering
  \resizebox{\linewidth}{!}{%
    \input{figures/arch_v2_tikz.tex}%
  }
...
\textbf{Input:} passing trace $P = (p_0,\ldots,p_{n-1})$ and failing trace $F = (f_0,\ldots,f_{m-1})$ for the \emph{same} test case---a reference execution and a run where one or more steps executed incorrectly or produced wrong observable state.
...
\textbf{Navigation signals} detect wrong-page loads: expected URL absent and unintended page present (\path{wrong_navigation}), boosting steps that explain silent navigation faults.

\textbf{Noise filtering} removes placeholder domains (e.g., \path{example.com} distractors) and known telemetry patterns so relative diffs focus on test-relevant traffic.

\textbf{Pixel diff} (local, no API) compares pass/fail PNGs for the heuristic top-5; enabled by default, disabled with \path{--no-pixel}.

Table~\ref{tab:signals} summarizes signals; the baseline mode is \path{python main2.py} without flags.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}P{0.15\linewidth}P{0.33\linewidth}P{0.23\linewidth}@{}}
    \toprule
    ...
    Navigation & Wrong page & Wrong click \\
    Pixel & Screenshot diff & Visual change \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{VLM analysis} sends pass/fail pairs for top-$K$ candidates to a vision-language model, returning per-step visual scores and a suggested root step.
...
\paragraph{Why top-15 for the LLM pool.}
Our traces contain roughly 20--40 aligned steps.
Sending all steps to the LLM (\texttt{pre\_llm\_k}${=}0$) increases token cost and context length on free-tier APIs without improving aggregate Hit@1 on our benchmark.
Conversely, restricting the pool to heuristic top-5 would miss causal steps that score low on text alone (e.g., silent navigation faults later promoted by the LLM).
\textbf{Top-15} balances recall and cost: it covers the heuristic shortlist while staying within prompt limits.
Steps outside the top-15 can still enter the pool via \emph{injection} when they carry strong relative signals (navigation mismatch, action change, errors on the next step, visual-causal root).
This design mirrors test-information visualization work that highlights a focused subset of execution events rather than entire traces~\cite{jones2002visualization}.
The value is fixed in \texttt{config.yaml} before evaluation.

\paragraph{Why top-5 for VLM screenshot pairs.}
VLM inference is the dominant API cost: with \texttt{per\_step=true}, each analyzed step requires one multimodal call per trace.
We set \texttt{top\_k\_for\_vlm}${=}5$ to match the final ranked output size (\texttt{top\_k}${=}5$) and our Hit@5 metric, while keeping daily free-tier quotas feasible on 22 traces.
Visual-causal divergence steps are injected into the VLM pool even when they fall outside the top-5 heuristic band, analogous to LLM injection.
Section~\ref{sec:threats} notes that incomplete screenshot artifacts (e.g., \texttt{dictionary}) can prevent VLM analysis of the full top-5 pool.

\paragraph{Hybrid ordering (LLM then VLM).}
Hybrid mode (\texttt{--llm --vlm}) runs the LLM reranker \emph{before} the VLM API call so both \texttt{llm\_prior} and \texttt{vlm\_prior} are ready at fusion time; the two model calls are sequential, not parallel.
In \textbf{v2}, however, the VLM still selects screenshot pairs from the \textbf{heuristic} top-$K$ (plus visual-causal injects), not from the LLM rerank list: the LLM does not directly narrow which PNG pairs the VLM sees.
Instead, both modalities contribute soft priors to the same fusion pool (Section~\ref{sec:fusion}).
v1's stacked pipeline (\texttt{main.py}) \emph{did} pass the LLM-hard-ranked top-$K$ to the VLM---we removed that hard feed-forward in v2 to keep ranking policy in one fusion function rather than a chained override.
...
\paragraph{Fusion weight selection (not benchmark-tuned).}
We did \textbf{not} grid-search the LLM/VLM prior weights on the 22 labeled traces.
Both values were fixed in \path{v2/config.yaml} \emph{before} aggregate reporting, using a principled carry-over from v1's hybrid ensemble.
In v1, the final LLM+VLM blend used \path{ensemble_vlm_weight=0.4}: each step's hybrid score was $60\%$ LLM rank position and $40\%$ VLM visual score ($1-w$ vs.\ $w$).
v2 maps the same \textbf{60/40 LLM--VLM ratio} onto the model-prior channels (\path{llm_prior=0.28}, \path{vlm_prior=0.18}), so ${\approx}61\%$ of the \emph{model-prior budget} goes to the LLM and ${\approx}39\%$ to the VLM.
Heuristic channels (text, navigation, pixel, etc.) use separate fixed weights from v1 development experience; only the LLM/VLM split is inherited from the established ensemble policy.
Section~\ref{sec:threats} discusses this as an internal-validity constraint.
...
We compare five configurations from the same v2 ranker (Section~\ref{sec:eval} reports outcomes):
\begin{itemize}
  \item \textbf{Heuristic (baseline):} pass--fail diff only; no LLM/VLM calls (\texttt{python main2.py}).
  \item \textbf{No pixel:} baseline without local screenshot diff (\path{--no-pixel}).
  \item \textbf{LLM:} adds text reranking and an \texttt{llm\_prior} channel (\texttt{--llm}).
  \item \textbf{VLM:} adds screenshot-pair analysis and a \texttt{vlm\_prior} channel (\texttt{--vlm}).
  \item \textbf{Hybrid:} all channels active (\texttt{--llm --vlm}).
\end{itemize}
...
\begin{figure}[t]
  ...
  \resizebox{0.92\linewidth}{!}{%
    \input{figures/arch_v1_tikz.tex}%
  }
...
Tuning for one pattern hurt accuracy on others; e.g., blocking deterministic promotes under visual lock sharply dropped accuracy on action-labeled traces in an intermediate experiment.
...
\textbf{RQ1:} How much do LLM, VLM, and hybrid fusion improve relative step ranking over the heuristic baseline?

\textbf{RQ2:} What is the contribution of pixel/visual channels (no-pixel ablation)?

\textbf{RQ3:} How do LLM-only and hybrid compare across Hit@$k$ and rank-quality metrics?

\textbf{RQ4:} How does performance vary across data sources and fault signal types?

\subsection{Benchmark and setup}

We evaluate on \textbf{22 traces} from three independent collections: 8 from efe/irem, 10 from areeb/salem, and 4 from ersel.
The ersel traces use a third schema (directory-based \path{steps.json}, session-wide console/network logs, and per-step \emph{intent} strings).
We label ersel separately in figures because its collector format differs from the other two sources; this is \emph{not} a train/test split---all 22 traces contribute to aggregate metrics.
Ground truth specifies one fault step per trace.
Table~\ref{tab:benchmark} summarizes fault-type frequencies; the full trace catalog is in our repository (Section~\ref{sec:data}).
...
All main results use v2 (\path{main2.py}) with frozen run JSONs listed in \path{scripts/metrics_manifest.yaml}.
Baseline: \path{python main2.py}.
\textbf{Models (free tier).}
LLM: Nemotron-3 Super~120B on OpenRouter ($T{=}0$).
VLM: Llama-4 Scout~17B on Groq ($T{=}0$), five screenshot pairs per trace (Section~\ref{sec:llm}).
Full provider model IDs are in \path{config.yaml}.
...
\textbf{RQ3.} Figures~\ref{fig:tradeoff} and~\ref{fig:radar} compare LLM and hybrid directly.
\textbf{LLM achieves the highest top-1 rate} (86\% Hit@1; 19/22 traces).
\textbf{Hybrid is stronger on overall ranking quality}: 100\% Hit@3 (vs.\ 95\%), lower mean rank distance (0.27 vs.\ 0.32), and lower MAD@5 (6.07 vs.\ 6.94).
Neither mode dominates every metric; per-trace disagreements (e.g., \texttt{amazon}, \texttt{github}) are analyzed in Section~\ref{sec:case}.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig6_llm_vs_hybrid_tradeoff.png}
  \caption{LLM vs.\ hybrid (v2, 22 traces). LLM leads on Hit@1 (top-1 localization); hybrid leads on Hit@3, mean rank distance, and MAD@5 (overall rank quality).}
...
\begin{figure}[t]
  ...
  \caption{Multi-metric radar (v1 and v2). On the v2 panel, LLM extends furthest on Hit@1; hybrid leads on Hit@3/5, rank distance, and MAD@5.}
  \Description{Radar chart comparing aggregate metrics for v1 and v2 configurations.}
  \label{fig:radar}
\end{figure}

Figure~\ref{fig:hitk} shows Hit@$k$ for all five configurations; Figure~\ref{fig:radar} summarizes multi-metric profiles.
...
Table~\ref{tab:main} and Figures~\ref{fig:tradeoff}--\ref{fig:radar} show that no configuration dominates every metric.
The heuristic baseline (64\% Hit@1) is fast and API-free but misses causal and screenshot-primary faults.
\textbf{LLM-only} achieves the highest top-1 rate (86\% Hit@1) and suits workflows that need a single best suspect.
\textbf{Hybrid} is strongest on overall ranking quality (100\% Hit@3, lowest mean rank distance and MAD@5) and suits workflows that need a robust shortlist.
\textbf{VLM-only} is competitive on top-$k$ but weaker alone at top-1; we treat it primarily as a hybrid component given API cost.
\textbf{No-pixel} ablation shows marginal aggregate gain from local screenshot diff but clear value on screenshot-primary traces (Section~\ref{sec:case}).
...
Fusion weights were fixed in \path{v2/config.yaml} before aggregate reporting; LLM/VLM prior weights inherit v1's 60/40 ensemble ratio (\path{ensemble_vlm_weight=0.4}) rather than being tuned on ground-truth ranks.
...
ersel (4 traces) uses intent fields and a different on-disk schema than the other collectors; we report it separately in by-source figures for transparency, not as a formal generalization test.
...
Relative debugging of failed UI test executions benefits from multimodal pass--fail comparison.
...
Against a 64\% heuristic baseline, results show LLM-only achieves the best top-1 localization (86\% Hit@1) while hybrid fusion is strongest overall (100\% Hit@3, lowest rank distance and MAD@5).
...
Source code, installation instructions, ground truth, the v2 pipeline (\path{main2.py}), frozen metric JSONs, and figure scripts are available at \repoLink.
Regenerate figures with \path{python scripts/plot_metrics.py}.
See \path{INSTALL.md} and \path{docs/DATA_LAYOUT.md} in the repository for setup and trace layout.
