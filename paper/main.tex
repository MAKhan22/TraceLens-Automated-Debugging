...
We did \textbf{not} grid-search \texttt{llm\_prior} or \texttt{vlm\_prior} on the 22 labeled traces.
Both values were fixed in \texttt{v2/config.yaml} \emph{before} aggregate reporting, using a principled carry-over from v1's hybrid ensemble.
In v1, the final LLM+VLM blend used \texttt{ensemble\_vlm\_weight}${=}0.4$ in \texttt{config.yaml}: each step's hybrid score was $60\%$ LLM rank position and $40\%$ VLM visual score ($1-w$ vs.\ $w$).
v2 maps the same \textbf{60/40 LLM--VLM ratio} onto the model-prior channels: \texttt{llm\_prior}${=}0.28$ and \texttt{vlm\_prior}${=}0.18$, so $0.28/(0.28+0.18)\approx 61\%$ of the \emph{model-prior budget} goes to the LLM and $39\%$ to the VLM.
Heuristic channels (text, navigation, pixel, etc.) use separate fixed weights chosen from v1 development experience; only the LLM/VLM split is inherited from the established ensemble policy.
...
\begin{itemize}
  ...
  \item \textbf{No pixel:} baseline without local screenshot diff (\texttt{--no-pixel}).
...
The ersel traces use a third schema (directory-based \texttt{steps.json}, session-wide console/network logs, and per-step \emph{intent} strings) and are labeled separately in figures as \emph{held-out} only in the sense of a distinct collector---not a statistically held-out train/test split (all 22 traces contribute to aggregate metrics).
...
VLM: \texttt{meta-llama/llama-4-scout-17b-16e-instruct} via Groq ($T{=}0$), five screenshot pairs per trace (\texttt{top\_k\_for\_vlm}, Section~\ref{sec:llm}).
...
Fusion weights were fixed in \texttt{v2/config.yaml} before aggregate reporting; \texttt{llm\_prior}/\texttt{vlm\_prior} inherit v1's 60/40 LLM--VLM ensemble ratio (\texttt{ensemble\_vlm\_weight}${=}0.4$) rather than being tuned on ground-truth ranks.
