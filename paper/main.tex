...
Ground truth specifies one fault step per trace covering network errors, authentication failures, console crashes, screenshot-only faults, absence-of-activity faults, and form/input faults.
Table~\ref{tab:benchmark} lists a representative subset; the full catalog is in our repository.

\begin{table*}[t]
  \caption{Representative benchmark traces (22 total). GT = ground-truth fault step (0-indexed).}
  \label{tab:benchmark}
  \small
  \begin{tabular}{llrll}
    \toprule
    Trace & Source & GT & Fault type & Primary signal \\
    \midrule
    hackernews & areeb/salem & 12 & Firebase timeout & Console + network \\
    amazon & areeb/salem & 20 & Auth-gated action & Console + action \\
    github & areeb/salem & 23 & JS rendering crash & Console \\
    saucedemo\_2 & efe/irem & 27 & Missing zipcode & Screenshot only \\
    wolfram & efe/irem & 12 & Wrong element clicked & Absence of activity \\
    wikipedia & efe/irem & 13 & Broken anchor / TOC & Network noise \\
    opencart\_wishlist\_35 & ersel & 22 & HTTP 404 product & Intent + network \\
    \bottomrule
  \end{tabular}
\end{table*}
...
LLM: Nemotron-3-Super-120B via OpenRouter ($T{=}0$).
VLM: Llama-4-Scout-17B via Groq ($T{=}0$), ${\sim}5$ screenshot pairs per trace.
...
\begin{table}[t]
  \caption{v2 aggregate results on 22 traces. Baseline = heuristic with pixel. VLM $n{=}21$ (\texttt{dictionary} excluded: all-zero visual scores).}
  \label{tab:main}
  \small
...
\begin{figure}[t]
  ...
  \caption{Per-trace Hit@1 heatmap (v2). Left band: source collection (ersel = test set).}
...
ersel (4 traces) uses intent fields absent elsewhere; VLM aggregate excludes \texttt{dictionary} ($n{=}21$).
